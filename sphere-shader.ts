...
vec3 calc( vec3 pos ) {
  ...
  // Reduced multipliers from 1.0 to 0.3 for a more subtle and contained energy effect
  return pos +
    0.3 * inputData.x * inputData.y * dir * (.5 + .5 * sin(inputData.z * pos.x + time)) +
    0.3 * outputData.x * outputData.y * dir * (.5 + .5 * sin(outputData.z * pos.y + time))
  ;
}
