...
export class GdmLiveAudioVisuals extends LitElement {
  ...
  // Use @query decorator to safely reference the canvas element and avoid TS shadowRoot errors
  ...
  protected firstUpdated() {
    // Fix: Removed manual shadowRoot access to resolve property error; canvas is now provided by @query
    this.canvas.width = 400;
    this.canvas.height = 400;
    this.canvasCtx = this.canvas.getContext('2d')!;
  }

  // Changed to public to match LitElement's render method access modifier and fix the TypeScript error
  render() {
